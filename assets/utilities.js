...
Shopify.theme.breadcrumbs = {
  init: function init(pages) {
    ...
    if (pages > 1) {
      ...
      if ( document.querySelector('.js-breadcrumb-text') ) {
...
Shopify.theme.quantityBox = {
  init: function init() {
    $('body').on('click', '[data-update-quantity]:not([disabled])', function () {
      console.log("[data-update-quantity]");
  ...
  updateQuantity: function updateQuantity($el) {
    ...
    var product_form = $quantityBox.parents(".shopify-product-form");
    ...
    if ($el.data('update-quantity') === 'plus') {
      ...
      if (val < valMax) {
        val++;
        $input.val(val);
        product_form.find(".quantity-input").val(val);
        console.log(product_form.find(".quantity-input"));
      }
...
var selectCallback = function selectCallback(variant, selector) {
  ...
  if ( $('.bsub-widget__plan-pricing').length ) {
    var cents = variant.price;
    var dollars = cents / 100;
    dollars = dollars.toLocaleString("en-US", {style:"currency", currency:"USD"});
    $('span[data-bsub-per-delivery-price]').html(dollars);
  }
